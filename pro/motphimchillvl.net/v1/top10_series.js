// Story 1-10 (updated 5-7a) | Motchill | Top 10 Phim Bộ Hot
// Target: https://motphimchillvl.net (homepage → most-view section)
// If html is provided (homebase HTML sharing), parse from it directly.

async function getTop10Series(html = null) {
    ...
    try {
        console.log('[KENG][1-10][Motchill] getTop10Series() html=' + (html ? 'provided(' + html.length + ')' : 'null'));
        ...
        while ((lm = liRe.exec(section)) !== null && movies.length < 10) {
            ...
            if (!hrefM) continue;
            movies.push({
                rank: rankM ? parseInt(rankM[1]) : movies.length + 1,
                title: hrefM[3], title_original: '', poster_url: '',
                url: hrefM[1], media_type: 'series', slug: hrefM[2],
                badge_text: viewM ? viewM[1].trim() : '', badge_sub: '',
                year: '', rating: '', synopsis: '', age_rating: '',
                episode_current: '', genres: []
            });
        }
        if (movies.length === 0) throw new Error('No movies parsed from most-view section');

        // Build slug→poster from article.item on homepage
        const artRe = /<article[^>]+class="[^"]*item[^"]*"[^>]*>([\s\S]*?)<\/article>/gi;
        const posterMap = {};
        let am;
        while ((am = artRe.exec(html)) !== null) {
            const block = am[1];
            const hm = block.match(/href="(?:https?:\/\/[^"]+)?\/phim\/([^"/?]+)/);
            const im = block.match(/data-original="(\/storage\/[^"]+)"/);
            if (hm && im) posterMap[hm[1]] = MC_BASE + im[1];
        }

        const misses = [];
        for (const m of movies) {
            if (posterMap[m.slug]) m.poster_url = posterMap[m.slug];
            else misses.push(m);
        }
        ...
        console.log('[KENG][1-10][Motchill] SUCCESS: ' + output.length + ' items, first: ' + output[0].title);
        return JSON.stringify(output);

    } catch (e) {
        console.log('[KENG][1-10][Motchill] ERROR: ' + e.message);
